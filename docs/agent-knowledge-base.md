...
[2026-03-13][devcontainer] The repository dev container is based on `mcr.microsoft.com/devcontainers/typescript-node:1-20-bookworm` and preinstalls the Electron shared libraries plus `xvfb` so extension tests run in containers without manual apt setup.
